...
import { Send, Sparkles, Terminal, Zap, Brain, Shield, Eye, Loader2 } from 'lucide-react';
import { useSound } from '@/hooks/d98/useSound';
import { chatD98 } from '@/ai/flows/d98-chat';
...
const CenterPanel = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const response = await chatD98({ prompt: userMessage });
        playReceive();
        setMessages((prev) => [...prev, { role: 'ai', content: response.response }]);
      } catch (error) {
        console.error("AI chat error:", error);
        playReceive();
        setMessages((prev) => [...prev, { role: 'ai', content: "ERROR: Connection to AI core lost. Please check console." }]);
      } finally {
        setIsTyping(false);
      }
    });
          ...
          <div className="glass-panel rounded-2xl p-1 border border-primary/30 hover:border-primary/50 focus-within:border-primary focus-within:shadow-neon transition-all">
            <div className="flex items-center gap-3 p-3 md:p-4">
              ...
              <motion.button
                ...
                style={{
                  boxShadow: input.trim() && !isPending ? '0 0 20px #ff0000' : undefined,
                }}
              >
